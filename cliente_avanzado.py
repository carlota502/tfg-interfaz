    ...
    try:
        ...
        if respuesta.status_code == 200:
            ...
            log_resultado = (
                ...
                f"- Estado: 200 OK\n"
                f"- Tiempo de cómputo: {tiempo_total:.2f} s\n"
                f"- Resolución activa: {ancho}x{alto} px\n"
                f"- Semilla aplicada: {semilla}"
            )
            return imagen_final, log_resultado
        else:
            return None, f"Error del servidor. Código: {respuesta.status_code}"
...
with gr.Blocks(title="TFG: Ingeniería de Sonido e Imagen") as mi_cliente:
    ...
    <div style='text-align: left; padding-bottom: 20px; border-bottom: 1px solid #e4e4e7; margin-bottom: 30px;'>
        ...
        <p style='margin: 6px 0 0 0; color: #71717a; font-size: 14px; font-weight: 400;'>Ajuste de parámetros y síntesis de imagen local</p>
    </div>
    """)
    
    with gr.Row():
        with gr.Column(scale=1):
            gr.Markdown("### Configuración de la Señal de Entrada")
            
            caja_prompt = gr.Textbox(
                label="Prompt de Entrada (Descriptor Textual)", 
                placeholder="Escriba la descripción de la imagen que desea generar...",
                lines=2
            )
            ...
            with gr.Accordion("Parámetros de Control Avanzado", open=True):
                deslizador_steps = gr.Slider(
                    minimum=1, maximum=50, value=15, step=1, 
                    label="Pasos de Desruido (Steps)",
                    info="Menos pasos reducen exponencialmente el tiempo de cómputo."
                )
                
                deslizador_cfg = gr.Slider(
                    minimum=1.0, maximum=20.0, value=7.0, step=0.5, 
                    label="Factor de Guía (CFG Scale)",
                    info="Fidelidad matemática respecto al texto de entrada."
                )
                ...
                selector_semilla = gr.Number(
                    ...
                    label="Semilla (Seed)", 
                    precision=0,
                    info="Usa -1 para aleatorio. Usa un número fijo para replicar resultados."
                )
        ...
        with gr.Column(scale=1):
            ...
            consola_telemetria = gr.Textbox(
                ...
                interactive=False,
                lines=5,
                placeholder="Esperando ejecución del pipeline..."
            )
